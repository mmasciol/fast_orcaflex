   ! ---------------------------------------------------
   ! MODIFIED BY PAUL FLEMING FOR HYWIND BLADED V3.8 DLL 
   ! ---------------------------------------------------
...
INTEGER(4), PARAMETER        :: Ptch_Cntrl        = 1                           ! Pitch control: 0 = collective, 1 = individual (-)

REAL(ReKi), PARAMETER        :: DTCntrl           = 0.007                      ! Communication interval for controller (sec) (this works in conjunction with FAST's time step (DT) the same way DTAero in AeroDyn works with FAST--see the FAST User's Guide description of DTAero for more information)
...
CHARACTER(1024), PARAMETER   :: DLL_ProcName      = 'DISCON'                    ! The name of the procedure in the DLL that will be called. STATOIL MOD: CHANGED TO CONTROL
...
INTEGER(4), PARAMETER        :: R                 = 145                         ! Start of below-rated torque-speed look-up table (record no.)
...
avrSWAP(66) = 120                                        ! Fix record 66 to be 120 (start of nacelle motions) (PF STATOIL CHANGE)
...
!Place the nacelle motions into the swap array (PF STATOIL CHANGE) 
!Motions
avrSWAP(120) = AllOuts(PtfmTDxi)
avrSWAP(121) = AllOuts(PtfmTDyi)
avrSWAP(122) = AllOuts(PtfmTDzi)
avrSWAP(123) = AllOuts(PtfmRDxi)
avrSWAP(124) = AllOuts(PtfmRDyi)
avrSWAP(125) = AllOuts(PtfmRDzi)

!Velocities:
avrSWAP(126) = AllOuts( NcIMUTVxs)
avrSWAP(127) = AllOuts( NcIMUTVys)
avrSWAP(128) = AllOuts( NcIMUTVzs)
avrSWAP(129) = AllOuts( NcIMURVxs)
avrSWAP(130) = AllOuts( NcIMURVys)
avrSWAP(131) = AllOuts( NcIMURVzs)

!Accelerations:
avrSWAP(132) = AllOuts( NcIMUTAxs)
avrSWAP(133) = AllOuts( NcIMUTAys)
avrSWAP(134) = AllOuts( NcIMUTAzs)
avrSWAP(135) = AllOuts( NcIMURAxs)
avrSWAP(136) = AllOuts( NcIMURAys)
avrSWAP(137) = AllOuts( NcIMURAzs)
