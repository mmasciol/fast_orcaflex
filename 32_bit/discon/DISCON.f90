...
IF ( iStatus == 0 )  THEN  ! .TRUE. if were on the first call to the DLL
   ...
   aviFAIL  = 1
   cMessage = 'Running with torque and pitch control of the NREL offshore 5MW baseline wind               turbine from DISCON.dll as written by J. Jonkman of NREL/NWTC for use in IEA                   Annex XXIII OC3 studies. Includes modifications for the Hywind spar.'   ! // & JASON:CHANGE MESSAGE FOR HYWIND:              'studies.'
